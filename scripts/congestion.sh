#!/usr/bin/env bash
# Switch real congestion on and off: ct1 (in upstream A) streams 10.5 Mbit/s of UDP
...
case "${1:-}" in
  start)
    docker exec -d ${CT} iperf3 -u -b 10.5M -t 86400 -c 10.40.10.10
    echo "Congestion running: 10.5 Mbit/s cross traffic on the transit link towards dest-1."
    ;;
  stop)
