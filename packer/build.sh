...
case "${TARGET}" in
  arm64)
    ...
    packer build -var-file=pkrvars/arm64.pkrvars.hcl -only="qemu.measlab-arm64" .
    ;;
  x86-vbox)
    echo "==> Building x86_64 Appliance (VirtualBox OVA)..."
    packer build -var-file=pkrvars/x86_64.pkrvars.hcl -only="virtualbox-iso.measlab-x86_64" .
    ;;
  x86-qemu)
    echo "==> Building x86_64 Appliance (QEMU / KVM)..."
    packer build -var-file=pkrvars/x86_64.pkrvars.hcl -only="qemu.measlab-x86_64" .
    ;;
  all)
