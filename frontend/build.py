...
<div class="status-bar" id="status-bar">
  <div class="status-pill"><span class="status-dot dot-gray" id="dot-cong"></span> Congestion: <strong id="lbl-cong">checking...</strong></div>
  <div class="status-pill"><span class="status-dot dot-gray" id="dot-peer"></span> IXP Peering: <strong id="lbl-peer">checking...</strong></div>
  <div class="status-pill"><span class="status-dot dot-gray" id="dot-scen"></span> Fault Scenarios: <strong id="lbl-scen">checking...</strong></div>
  <button class="portal-btn" id="btn-refresh-status" style="margin-left:auto;padding:4px 10px;font-size:12px;">Refresh</button>
</div>
...
<div class="console-grid">
  <div class="console" data-port="7681">
    <div class="console-head">
      ...
      <a href="#" target="_blank" class="console-action" data-act="popout">pop out</a>
    </div>
    <div class="console-resizer"><iframe src="about:blank" title="host1 terminal"></iframe></div>
  </div>
  <div class="console" data-port="7682">
    ...
    <div class="console-resizer"><iframe src="about:blank" title="r1 terminal"></iframe></div>
  </div>
  <div class="console" data-port="7683">
    ...
    <div class="console-resizer"><iframe src="about:blank" title="r2 terminal"></iframe></div>
  </div>
  <div class="console" data-port="7684">
    ...
    <div class="console-resizer"><iframe src="about:blank" title="r3 terminal"></iframe></div>
  </div>
...
(function () {
  ...
  // Set up dynamic terminal URLs based on current host & environment
  var hostname = window.location.hostname || "localhost";
  var proto = window.location.protocol;
  document.querySelectorAll(".console[data-port]").forEach(function (con) {
    var port = con.dataset.port;
    var url;
    if (hostname.endsWith(".app.github.dev") && hostname.includes("-8080")) {
      url = proto + "//" + hostname.replace("-8080", "-" + port);
    } else {
      url = proto + "//" + hostname + ":" + port;
    }
    var iframe = con.querySelector("iframe");
    var popout = con.querySelector('a[data-act="popout"]');
  ...
  function updateStatus() {
    fetch("/api/status").then(function (r) { return r.json(); }).then(function (res) {
      if (!res.ok) return;
      var lblCong = document.getElementById("lbl-cong");
      var dotCong = document.getElementById("dot-cong");
      var lblPeer = document.getElementById("lbl-peer");
      var dotPeer = document.getElementById("dot-peer");
      var lblScen = document.getElementById("lbl-scen");
      var dotScen = document.getElementById("dot-scen");

      if (lblCong) {
        lblCong.textContent = res.congestion === "running" ? "Active (10.5M)" : "Stopped";
        dotCong.className = "status-dot " + (res.congestion === "running" ? "dot-warn" : "dot-ok");
      }
      if (lblPeer) {
        lblPeer.textContent = res.peering === "active" ? "Enabled (IXP)" : "Disabled (Transit)";
        dotPeer.className = "status-dot " + (res.peering === "active" ? "dot-ok" : "dot-gray");
      }
      if (lblScen) {
        if (res.scenario3 === "on") {
          ...
          dotScen.className = "status-dot dot-ok";
        }
      }
    }).catch(function () {});
  }
...
.status-bar {
  ...
  padding: 10px 14px; margin: 14px 0 20px; font-family: var(--mono); font-size: 13px;
}
.status-pill { display: flex; align-items: center; gap: 8px; color: var(--text); }
.status-dot { width: 9px; height: 9px; border-radius: 50%; display: inline-block; flex: none; }
.dot-ok { background: #4cd964; box-shadow: 0 0 6px #4cd96488; }
.dot-warn { background: #f26b21; box-shadow: 0 0 6px #f26b2188; }
.dot-err { background: #f36b6b; box-shadow: 0 0 6px #f36b6b88; }
.dot-gray { background: #6b7796; }
